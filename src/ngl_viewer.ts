...
declare namespace NGL {
    ...
    interface StageSignals {
        clicked: {
            add(callback: (pickData: PickData) => void): void;
        };
    }

    class Stage {
        signals: StageSignals;
        constructor(elementId: string, params?: StageParameters);
        handleResize(): void;
        loadFile(file: Blob, params?: { ext?: string }): Promise<Component>;
    }
}

// Type definitions for QWebChannel
...
interface Bridge {
    sendMessage: {
        connect(callback: (pdbText: string) => void): void;
    };
    fromJs(message: string): void;
}
...
interface Window {
    bridge?: Bridge;
    _last_pdb_text?: string;
    loadPDBFromText?: typeof loadPDBFromText;
    highlightAtoms?: typeof highlightAtoms;
}

// Helper function to send log messages to Python
function sendLog(message: string, data?: any): void {
    const logInfo = {
        type: "log",
        message: message,
        data: data
...
let currentComponent: NGL.Component | null = null;

// Function to extract unique residue names from PDB text
function extractUniqueResnames(pdbText: string): string[] {
...
function generateColorScheme(resnames: string[]): { [key: string]: string } {
...
function loadPDBFromText(pdbText: string): void {
    ...
    stage.loadFile(blob, { ext: "pdb" }).then(function (o: NGL.Component) {
        currentComponent = o;
...
let selectionRepresentation: any = null;

interface HighlightData {
    serials: number[];
}

function highlightAtoms(data: HighlightData): void {
    sendLog("highlightAtoms called", { serials: data.serials });

    if (selectionRepresentation) {
        try {
            currentComponent?.removeRepresentation(selectionRepresentation);
        } catch (e) {
            // Ignore errors during cleanup
        }
...
new QWebChannel(qt.webChannelTransport, function (channel: QWebChannel) {
    ...
    window.bridge.sendMessage.connect(function (pdbText: string) {
...
stage.signals.clicked.add(function (pickData: NGL.PickData) {
...
(window as any).loadPDBFromText = loadPDBFromText;
(window as any).highlightAtoms = highlightAtoms;
